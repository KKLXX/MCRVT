...
from utils import _get_activation_fn, arbitrary_segment_mask, arbitrary_point_mask, inverse_sigmoid
...
class FFN(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class Versatile_attention(nn.Module):
...
class VA_Encoder(nn.Module):
    def __init__(self, embed_dim, ffn_embed_dim, num_heads, length, duration_init_min, duration_init_max, dropout=0., bias=True, activation='relu'):
        ...
        self.attention = Versatile_attention(embed_dim, num_heads, length, duration_init_min, duration_init_max, dropout, bias)
